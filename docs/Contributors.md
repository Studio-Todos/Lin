...
4. **No `any` types:** The use of MLIR `any` types (e.g., `AnyType`, `AnyInteger`, `AnyFloat`, or untyped/erased type parameters in TableGen) is **banned** across the entire codebase. All types must be concrete and explicitly specified. Prefer `i64` as the canonical integer wire type throughout the interaction net pipeline. Using `any` leads to ambiguous lowering, breaks the type-directed dispatch in `LoweringPasses.cpp`, and undermines the compiler's ability to statically verify reduction correctness.
