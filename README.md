...
Welcome to the documentation for **Lin**, which serves as a general-purpose, heterogeneous programming language. Lin is a multi-paradigm, statically typed language built on top of **Polarized Interaction Combinators Reversible (PICR)** to achieve massively parallel, optimal, and reversible evaluation.
...
> [!NOTE]
> This is an experimental language actively developed with the help of AI tooling; expect breaking changes as the language matures.
...
3. **Predictable Performance:** Utilize lock-free Ahead-Of-Time (AOT) execution mechanisms. The compiler translates the interaction net into dedicated MLIR dialects (`pic.graph`, `pic.reduce`, `pic.runtime`) representing a flat, pre-allocated memory array compiled directly to native machine code.
...
* **[Architecture](docs/Architecture.md):** Dive deep into how Lin achieves optimal, reversible evaluation through Polarized Interaction Combinators Reversible (PICR), MLIR, and 128-bit atomic memory arrays.
