...
Welcome to the documentation for **Lin**, a "General Purpose Heterogeneous Programming Language". Lin is a multi-paradigm, statically typed language built on top of Polarized Interaction Combinators (PIC) to achieve massively parallel, optimal evaluation.

Lin takes inspiration from the Red/Rebol syntax family (as well as [daslang](https://daslang.io/), Microsoft's Verona, and unified algebra systems) while aiming for unparalleled runtime performance through a novel compiler backend. By compiling directly to an Interaction Net model and generating highly optimized LLVM IR (leveraging MLIR), Lin code evaluates via a lock-free, addressable 128-bit atomic array—enabling massive parallelization across CPU threads and GPU cores with zero locks. Because everything lowers to interaction nets, **everything that can run in parallel *will* run in parallel.**

## Goals

1. **Massively Parallel Optimal Evaluation:** Execute graph-based programs without locks, drastically reducing synchronization overhead and leveraging massive core counts on CPU, GPU, and SHADERS.
2. **Predictable Performance:** Utilize lock-free Ahead-Of-Time (AOT) execution mechanisms. The compiler translates the interaction net into an MLIR dialect representing a flat, pre-allocated memory array compiled directly to native machine code.
...
5. **Language Philosophy:**
   ...
   - **Low abstraction** (leveraging the Red/Rebol syntax and build environment to make nearly everything configurable)
   - **Versatile syntax**
...
The Lin compiler is called **`linc`** (pronounced "link"). It compiles Lin source code into a single static binary called a `.line` (Lin executable), taking inspiration from Go's single static binaries and near-instant cross-compilation.

Future iterations of `linc` will feature:
* **Hot code reloading**
* **Built-in `checkstyle` equivalent**, run automatically for all projects during `linc test`.
